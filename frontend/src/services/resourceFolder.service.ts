...
class ResourceFolderService {
   ...
   * 获取文件夹列表
   */
  async getFolders(teacherId: number, parentId?: number): Promise<ResourceFolder[]> {
    const params: any = { teacher_id: teacherId };
    if (parentId !== undefined) {
      params.parent_id = parentId;
    }
    const response = await apiClient.get<ResourceFolder[]>(this.API_BASE_URL, { params });
   ...
   * 移动资源
   */
  async moveResource(resourceId: number, teacherId: number, targetFolderId?: number): Promise<any> {
    const params: any = { teacher_id: teacherId };
...
export default resourceFolderService;
