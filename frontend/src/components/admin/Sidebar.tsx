...
export default function Sidebar() {
  ...
  const getIcon = (iconName: string, isActive: boolean) => {
    ...
    const icons: { [key: string]: JSX.Element } = {
      ...
      enrollments: (
        <svg className="w-5 h-5" fill="none" stroke={color} viewBox="0 0 24 24" strokeWidth="2">
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m-7 4h8a2 2 0 002-2V7a2 2 0 00-2-2H8l-3 3v10a2 2 0 002 2z" />
        </svg>
      ),
      courseCovers: (
        <svg className="w-5 h-5" fill="none" stroke={color} viewBox="0 0 24 24" strokeWidth="2">
  ...
  const navItems = [
    ...
    { href: '/admin/users', label: t.nav.users, iconName: 'users' },
    { href: '/admin/dictionary', label: t.nav.dictionary, iconName: 'dictionary' },
    { href: '/admin/enrollments', label: t.nav.enrollments, iconName: 'enrollments' },
    ...
    { href: '/admin/llm-call-logs', label: t.nav.llmCallLogs, iconName: 'llmCallLogs' },
  ];
